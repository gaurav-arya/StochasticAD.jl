...
for dist in [:Geometric, :Bernoulli, :Binomial, :Poisson]
    @eval function Base.rand(rng::AbstractRNG,
        d_st::$dist{StochasticTriple{T, V, FIs}}; stochastic_ad_map_kwargs=(;)) where {T, V, FIs}
        ...
        Δs2 = map(map_func, st.Δs; enumeration, deriv = δ -> smoothed_delta(d, val, δ), out_rep = val, stochastic_ad_map_kwargs...)
...
function Base.rand(rng::AbstractRNG,
    ...
    Δs2 = map(map_func, Δs_coupled; enumeration, deriv = δ -> smoothed_delta(d, val, δ), out_rep = val, stochastic_ad_map_kwargs...)
