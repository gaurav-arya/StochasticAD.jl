...
# AbstractFIs are leaves; Functors 0.5 would otherwise recurse into their fields
structural_exclude(x) = Functors.isleaf(x) || (x isa AbstractFIs)
...
function structural_map(f, args...; only_vals = nothing)
    ...
    fmap((args...) -> args[1] isa AbstractArray ? f.(args...) : f(args...), args...;
        cache = nothing,
        walk, exclude = structural_exclude)
end
