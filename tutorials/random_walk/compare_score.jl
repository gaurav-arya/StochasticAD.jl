...
begin
    ...
    @showprogress for (n, p) in zip(RandomWalkCore.n_range, RandomWalkCore.p_range)
        ...
        std_smoothed = std(derivative(p -> RandomWalkCore.fX(p,n; hardcode_leftright_step = true), p)
                        for i in 1:(RandomWalkCore.nsamples))
...
begin
    ...
    if show_smoothed
        plot!(RandomWalkCore.n_range, stds_smoothed, color = 4, label = "smoothed stochastic triples")
